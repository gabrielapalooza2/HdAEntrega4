"""Pruebas del BFF. Ninguna necesita un broker vivo.

Se sustituye el publicador por un doble que guarda lo que se le pidio publicar.
Eso permite comprobar la TRADUCCION (que es el trabajo real del BFF) sin montar
infraestructura: que los pesos se vuelvan centavos, que las fechas se vuelvan
milisegundos, que el correlation_id viaje en el sobre y que cada comando salga
al topico correcto.
"""
...
from bff import config                                    # noqa: E402
from bff.infraestructura import publicador as mod_pub     # noqa: E402
from bff.infraestructura.operaciones import ACEPTADA, COMPLETADA, RECHAZADA, registro  # noqa: E402
from bff.infraestructura.proyeccion import proyeccion     # noqa: E402
...
# ── traduccion de la API al contrato ─────────────────────────────────────────
...
def test_los_pesos_se_publican_como_centavos(cliente):
    """El contrato usa la unidad minima. Publicar 500000 en vez de 50000000 seria
    un error de tres ordenes de magnitud que nadie nota hasta produccion."""
...
def test_las_fechas_se_publican_como_milisegundos(cliente):
    ...
    assert payload['vigencia_hasta'] == 0        # sin fecha de fin
...
def test_la_clave_de_particion_es_la_del_agregado(cliente):
    """Dos comandos sobre el mismo partner deben ir a la misma particion, o
    pueden procesarse fuera de orden."""
...
def test_no_existe_endpoint_de_asignacion(cliente):
    """cmd.emparejamiento no lo consume NADIE: Emparejamiento reacciona a
    TrabajoCreado. Un endpoint que publicara ahi devolveria 202 sin efecto."""
...
# ── la proyeccion y la vista compuesta ───────────────────────────────────────
...
def test_vista_compuesta_reune_tres_servicios_en_una_respuesta(cliente):
    ...
    assert v["partner"]["nombre"] == "Andina"          # de Motor reglas partner
    assert v["asignacion"]["asignacion_id"] == "a1"    # de Emparejamiento
    assert v["proveedor"]["estado"] == "HABILITADO"    # de Acreditacion
...
def test_health_responde_sin_broker(cliente):
    """El BFF arranca y responde aunque Pulsar no este: eso es lo que permite
    que la sonda de GCP no lo mate durante el arranque del cluster."""
    r = cliente.get("/health")
    assert r.status_code == 200
    assert r.json()["estado"] == "ok"


# ── la saga, con los eventos que agrego el equipo en la entrega 5 ────────────
...
# ── el formato de cable, contra los .avsc REALES del equipo ─────────────────
