"""Lecturas: responden desde la proyeccion, sin tocar a ningun servicio."""
...
def estado_de_operacion(correlation_id: str):
    """El recurso que convierte una escritura asincrona en algo consultable.

    ACEPTADA   el comando esta en el bus, todavia no hay desenlace
    COMPLETADA llego el evento de exito
    RECHAZADA  llego un evento de rechazo o de compensacion
    """
...
def obtener_trabajo(trabajo_id: str):
    """La consulta que justifica que exista un BFF.

    Un cliente movil que quisiera esta misma pantalla sin BFF tendria que llamar
    a Orquestacion de trabajos, a Motor reglas partner y a Emparejamiento, y
    componer las tres respuestas. Aqui es una sola llamada, y el cliente no sabe
    cuantos servicios hay detras.
    """
...
def saga_de_un_trabajo(trabajo_id: str):
    """Derivada de los eventos que el BFF ya consume. NO llama a Orquestacion.

    El saga log AUTORITATIVO vive en el coordinador (`orquestacion-trabajos`),
    en las tablas `saga_asignacion` y `saga_paso`, y se consulta con SQL o por
    su `GET /sagas`. Esta vista es complementaria: le da al cliente la misma
    historia sin que el BFF tenga que hacer una llamada sincrona entre
    servicios, que es justo lo que el enunciado prohibe.
    """
