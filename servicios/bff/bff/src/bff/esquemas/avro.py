"""Codificacion y decodificacion Avro contra los .avsc del equipo.

POR QUE EL BFF NO COPIA LOS CONTRATOS
-------------------------------------
Los otros cuatro servicios copian las clases `Record` de `contratos/generar.py`.
El BFF no: lee directamente los `.avsc` versionados, montados en
`/app/contratos`. No hay una copia que se pueda quedar atras, y cuando el equipo
agrega un mensaje nuevo el BFF lo entiende sin recompilar.

POR QUE BytesSchema Y NO AvroSchema
-----------------------------------
Es la convencion del equipo y no es opcional: Orquestacion, Acreditacion y
Emparejamiento publican con `BytesSchema` y codifican el payload ellos mismos.
Si el BFF publicara con `AvroSchema(Clase)`, el broker intentaria registrar un
esquema AVRO en un topico donde los demas producen BYTES, y la creacion del
productor fallaria por conflicto de esquema.

Ademas hace falta por otra razon: `evt.trabajos` lleva TRES tipos de evento y
`evt.asignaciones` lleva CUATRO. Pulsar valida un solo esquema por consumidor,
asi que con AvroSchema no se podria leer un topico con varios tipos.

EL TRUCO DEL PREFIJO POSICIONAL
-------------------------------
Los ocho campos del sobre CloudEvents son identicos, en el mismo orden, en los
quince esquemas. Avro binario es posicional y no lleva nombres de campo en el
cable, asi que se puede leer SOLO el sobre sin saber todavia que tipo viene,
mirar `type`, y recien entonces decodificar el mensaje entero con el esquema
correcto. Dos pasadas sobre los mismos bytes en memoria: es barato.
"""
...
def decodificar(crudo: bytes) -> dict | None:
    ...
    # Mensajes JSON antiguos que siguen en los topicos por la retencion
    # infinita. Liberal al recibir, estricto al emitir: el BFF nunca emite JSON.
    if crudo[:1] == b"{":
        try:
